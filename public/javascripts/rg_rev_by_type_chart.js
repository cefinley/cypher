...
jQuery(document).ready(function() {
  ...
  var rev_piechart_options = {
    title: "Service Billing by Type: Last Month",
    cursor: {
  ...
  if (rev_by_type_data_points.length == 0 ) {
    // chart always wants a 2D array
    rev_by_type_data_points = [[]];
    rev_piechart_options.title = "No Service Billing for Types: Last Month";
  }


  var rev_by_type_chart = $.jqplot('rev_by_type_chart', [rev_by_type_data_points], rev_piechart_options);

  $('#rev_by_type_chart').data('current_chart', rev_by_type_chart);

  // for showing rev types on piechart mouse over
  $('#rev_by_type_chart').bind('jqplotDataMouseOver', 
                              function (ev, seriesIndex, pointIndex, data) {
                                var ttip = "<span >" + data[0] +", "+ data[1] + "<span>";
                                $('#rev_by_type_chart > .jqplot-cursor-tooltip').append(ttip).show();               
                              }
                             );

/*
  var options = {
    uri: rev_by_type_update_uri,
    chart: rev_by_type_chart,
    constrainDates: true,
  ...
  $('#rev_by_type_start_date_picker').daterangepicker(options);
  */  
});
